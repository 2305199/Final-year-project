...
function Home() {
  ...
  const handleScan = async () => {
    try {
      ...
      setScanTime(durationS);
      console.log(`Scan completed in ${durationS.toFixed(2)} ms`);
    } catch (err) {
      console.error(err);
    } finally {
      setScanning(false);
  ...
  const totalsAllTime = scans.reduce(
    (acc, scan) => {
      acc.total += scan.totalUrlsScanned || 0;
      acc.safe += scan.safeUrlsFound || 0;
      acc.phishing += scan.phishingUrlsFound || 0;
      return acc;
    },
    { total: 0, safe: 0, phishing: 0 }
  );

  // All-time chart data (oldest -> newest)
  const chartData = scans
    .slice(0, 10)
    .reverse()
    .map((scan) => ({
      name: scan.id,
      phishing: scan.phishingUrlsFound || 0,
      safe: scan.safeUrlsFound || 0,
    }));
    ...
    <div className={styles.page}>
      ...
      <section className={styles.Box}>
        <div className={styles.topRow}>
          ...
          <div className={styles.statCard}>
            <div className={styles.statLabel}>Safe URLs found</div>
            <div className={styles.statValue}>
          ...
          <div className={styles.statCard}>
            <div className={styles.statLabel}>Last scan time</div>
            <div className={styles.statValue}>
              {scanTime !== null ? `${scanTime.toFixed(2)} s` : "N/A"}
            </div>
          </div>
        </div>
      ...
      <section className={styles.section}>
        <div className={styles.bottomGrid}>
          ...
          <div className={styles.panel}>
            <h2 className={styles.panelTitle}>All-Time Analytics</h2>

            <div className={styles.analyticsCircles}>
              <div className={styles.metric}>
                <div className={styles.circle}>{totalsAllTime.total}</div>
                <div className={styles.metricLabel}>Total scanned</div>
              </div>

              <div className={styles.metric}>
                <div className={styles.circle}>{totalsAllTime.safe}</div>
                <div className={styles.metricLabel}>Safe</div>
              </div>

              <div className={styles.metric}>
                <div className={styles.circle}>{totalsAllTime.phishing}</div>
          ...
          {/* TRENDS GRAPH */}
          <div className={styles.panel}>
            <h3 className={styles.panelTitle}>Trends (All-Time)</h3>
