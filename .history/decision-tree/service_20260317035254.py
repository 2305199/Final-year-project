...
class UrlBatch(BaseModel):  
    urls: List[str]   # structure of post request list of urls

@app.post("/predict-batch")
def predict_batch(batch: UrlBatch): # receives list of urls
    results = []
    for url in batch.urls: # goes through urls
        feats_dict = extract_features(url) # extract the features
        row = [feats_dict[name] for name in feature_names] # create list same order as training
        pred = model.predict([row])[0] # prediction of each url 

        if str(pred).isdigit():
            pred_label = "phishing" if int(pred) == 1 else "legitimate" # labeling phishing or safe 1 or 0
        else:
            pred_label = str(pred)

        results.append({"url": url, "prediction": pred_label, "features": feats_dict}) #store results in results list

    return {"count": len(results), "results": results} # the results of prediction + count of urls predicted