...
def has_ip(host: str) -> int:
    return 1 if re.match(r"^\d{1,3}(\.\d{1,3}){3}$", host) else 0 # check if has IP 1 if yes 0 if no 
    # by checking if it matches IPV4 pattern xxx.xxx.xxx.xxx 

def entropy(s: str) -> float:  # function to calcuate how random hostname is
    if not s: 
        return 0.0 
    counts = {}
    for ch in s:
        counts[ch] = counts.get(ch, 0) + 1 # loops through each character in string sees how many it appears
    n = len(s) # the number of characters in string
    return entropy(list(counts.values()), base=2) # formula to calculate entropy
...
def main():
    ...
    out = pd.concat([feat_df, df[[LABEL_COL]].reset_index(drop=True)], axis=1)
